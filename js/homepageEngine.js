...
// ---- GLOBE RENDERER ----
const GlobeRenderer = (() => {
  let canvas, ctx, animId, rotation = 0;

  function init() {
    canvas = document.getElementById('globe-canvas');
    if (!canvas) return;
    const size = 320 * (window.devicePixelRatio || 1);
    canvas.width = size;
    canvas.height = size;
    ctx = canvas.getContext('2d');
    loop();
  ...
  function loop() {
    ...
    glow.addColorStop(0, 'rgba(56,136,232,0.12)');
    glow.addColorStop(0.5, 'rgba(56,136,232,0.04)');
    ...
    grad.addColorStop(0.7, 'rgba(20,60,120,0.2)');
    grad.addColorStop(1, 'rgba(4,6,10,0.8)');
    ctx.beginPath();
    ctx.arc(cx, cy, r, 0, Math.PI * 2);
    ctx.fillStyle = grad;
    ctx.fill();
    ctx.strokeStyle = 'rgba(56,136,232,0.25)';
    ctx.lineWidth = 1.5;
    ctx.stroke();

    // Wireframe meridians
    ctx.strokeStyle = 'rgba(56,136,232,0.12)';
    ctx.lineWidth = 0.8;
    for (let i = 0; i < 8; i++) {
    ...
    // Parallels
    for (let i = 1; i < 5; i++) {
      const py = cy + (i / 5 - 0.5) * 2 * r * 0.85;
      const pr = Math.sqrt(Math.max(0, r * r - (py - cy) * (py - cy)));
      ctx.beginPath();
      ctx.ellipse(cx, py, pr, pr * 0.15, 0, 0, Math.PI * 2);
      ctx.stroke();
    }
    ...
    for (let i = 0; i < dotCount; i++) {
      const theta = (i / dotCount) * Math.PI * 2 + rotation * 2;
      const phi = Math.acos(1 - 2 * ((i * 7.3 % dotCount) / dotCount));
      const x = cx + r * 0.85 * Math.sin(phi) * Math.cos(theta);
      const y = cy + r * 0.85 * Math.cos(phi);
      const z = Math.sin(phi) * Math.sin(theta);
      if (z < 0) continue; // behind globe
      const opacity = 0.15 + z * 0.4;
      ctx.beginPath();
      ctx.arc(x, y, 1 + z, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(56,136,232,${opacity})`;
      ctx.fill();
    }
